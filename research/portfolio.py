...
def simulate(trades, mid, index, broker=Broker(), risk_frac=0.01,
    ...
    for b in range(n):
        ...
        # ---- 5. margin call -----------------------------------------------
        if stopped_out_at is None and used_margin > 0 and \
                eq / used_margin < broker.margin_call_frac:
            stopped_out_at = b

    # -------------------------------------------------------- drawdowns --
    eqc = pd.Series(equity_curve).ffill().fillna(start_equity).to_numpy()
    run_max = np.maximum.accumulate(eqc)
    dd_floating = float(np.max(1.0 - eqc / np.maximum(run_max, 1e-9)))

    bal = np.array([start_equity] + [r["balance_after"] for r in log], float)
    bmax = np.maximum.accumulate(bal)
    dd_closed = float(np.max(1.0 - bal / np.maximum(bmax, 1e-9)))
...
def from_trades(W, R_mask, held, mid, index, spread, **kw):
    ...
    for q in idxs:
        ...
        trades.append(dict(entry_bar=e, exit_bar=x, d=int(W["d"][q]),
                           entry_px=float(W["entry"][q]), exit_px=float(mid[x]),
