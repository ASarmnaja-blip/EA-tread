...
STAGES = [
    ...
    ("passive_execution", "a resting order buys 7.3 round trips of adverse "
     "selection to save one", "passive_execution.json",
     [(("viable",), 0, 0)]),
    ("cheap_corner", "the edge turns down before the quote stops tightening",
     "cheap_corner.json",
     [(("viable",), 0, 0)]),
    ("financing_closure", "every horizon loses at zero carry",
     "financing_closure.json",
     [(("closed",), True, 0)]),
    ("reversal_anatomy", "the reversal is volatility-scaled, not bounce",
     "reversal_anatomy.json",
     [(("regression", "beta_atr"), 0.986, 0.03),
      (("regression", "beta_spread"), -0.103, 0.03)]),
    ("back_catalogue", "the catalogue survives its own correction",
     "back_catalogue.json",
     [(("clear",), 0, 0),
      (("rank_corr_R_vs_edge",), -0.05, 0.10)]),
